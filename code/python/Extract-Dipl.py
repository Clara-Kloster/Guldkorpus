...
for row in d:
    try:
        if row[None][8].strip() == "Dansk":
            charter = row[None][0].strip()
            danish.append(charter)
            danish_dates[charter] = row[None][5].strip()
            danish_places[charter] = row[None][14].strip()
    except:
        pass
danish.sort()
...
for item in danish:
    write_file.write("** " + item + " " + danish_places[item] + " " + danish_dates[item] + "\n")
